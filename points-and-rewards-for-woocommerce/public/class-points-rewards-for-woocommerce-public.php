...
class Points_Rewards_For_WooCommerce_Public {
	...
	public function mwb_wpr_get_social_shraing_section( $user_id ) {
		...
		do_action( 'mwb_wpr_insta_refer', $user_reference_key, $enable_mwb_social, $page_permalink );
		if ( $enable_mwb_social ) {

			$content  = '';
			$html_div = '<div class="mwb_wpr_wrapper_button">';
			...
			$maill = apply_filters( 'mwb_mail_box', $content, $user_id );
	...
	public function mwb_wpr_woocommerce_order_status_changed( $order_id, $old_status, $new_status ) {
		...
		// check allowed user for points features.
        $currency_Type = get_woocommerce_currency_symbol(); //Get USD - Akalanka
		if ( $old_status != $new_status ) {
			...
			$user_email = $user->user_email;
			if ( 'processing' == $new_status ) { //completed Mod By Akalanka
						...
						if ( $mwb_wpr_coupon_conversion_enable ) {
							if ( $conversion_points_is_enable_condition || ! $points_key_priority_high ) {
								...
								// 	remove (int) here
									...
									/*Get the value of the price - Moded By Akalanka*/
									if($currency_Type == 'USD'){
										$mwb_wpr_coupon_conversion_price = '5';
									}else{
...
// Akalanka - i can do floor() or (int)$var /added round - finally bcdiv;
									$points_calculation = bcdiv( ( $order_total * $mwb_wpr_coupon_conversion_points ) / $mwb_wpr_coupon_conversion_price, 1, 2);
									...
									/*Total Point of the order remove intval( - Akalanka*/ 
									$total_points = $points_calculation + $get_points;
		...
		if ( 1 == $mwb_wpr_custom_points_on_cart && isset( $user_id ) && ! empty( $user_id ) ) {
			...
			<div class="woocommerce-message"><?php esc_html_e( 'Here is the Discount Rule for Applying your Points to Cart Total', 'points-and-rewards-for-woocommerce' ); ?>
				<ul>
					...
					echo wp_kses( wc_price( 1000 ), $allowed_tags ) . ' = ' . esc_html( $mwb_wpr_cart_points_rate ) . esc_html__( ' Points', 'points-and-rewards-for-woocommerce' );
					?>
					</li>
				</ul>
			</div>
	...
	public function mwb_wpr_woocommerce_get_item_data( $item_meta, $existing_item_meta ) {
		...
		return 0; /*$item_meta removed -Akalanka*/
	}
	...
	public function mwb_wpr_woocommerce_content_change( $cart_contents ) {
		...
		public function mwb_wpr_woocommerce_order_status_cancel($order_id, $old_status, $new_status){	
			if($old_status != $new_status){
				if(($old_status == "completed" || $old_status == 'processing') && $new_status == 'refunded') {
					$order = wc_get_order( $order_id );
					$user_id = $order->get_user_id();
					$user=get_user_by('ID',$user_id);
						...
						foreach ( $coupons as $item_id => $item ) :
							foreach ($user_log as $key => $value) {
								if(in_array(strtoupper($item->get_code()), $value)) {
									...
									// $couponamont = $couponamont+$order_discount_tax_amount+$item->get_discount();
                    ...
                    if($currency_symbol == 'USD'){ //If USD Convert - Akalanka
						$mwb_wpr_coupon_conversion_price= '5';
					}else{
					...
					{	
						$get_points = get_user_meta($user_id, 'mwb_wpr_points', true); //remove int
						...
						$refund_amount = bcdiv( ( $refund_amount * $mwb_wpr_coupon_conversion_points ) / $mwb_wpr_coupon_conversion_price, 1, 2);
			...
			//Refund amount of Point Discount
				...
				if(($old_status == "processing" || $old_status == 'completed') && $new_status == 'cancelled') {
					$order = wc_get_order( $order_id );
					$user_id = $order->get_user_id();
					//========Start Return amount into the coupon========
					$user_log = get_user_meta($user_id, 'mwb_wpr_user_log', true);
								...
								if(!empty($order_fees)){
									foreach ( $order_fees as $fee_item_id => $fee_item ){
										...
										//Now deduct the fee amount from user account as the point
											$fee_amount = -($fee_amount);
											//$fee_to_point = ceil(($mwb_wpr_purchase_points * $fee_amount)/$mwb_wpr_product_purchase_price);
											$fee_to_point = bcdiv( ( $mwb_wpr_purchase_points * $fee_amount ) / $mwb_wpr_product_purchase_price, 1, 2);
											$total_point = $get_points + $fee_to_point;
